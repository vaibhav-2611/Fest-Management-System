private void update(int wm,String user,String hname,String roomlist){
...
//to change        
        try{
            String sql="Insert into Logindata (Username,Password,Name,Address,Email,Phoneno,Waletmoney,Activestatus,Designation,InstituteName,City,State,Moneydemand,HallName,RoomNo) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            pst= conn.prepareStatement(sql);
            pst.setString(1, Username);
            ...
            pst.setString(14, Hall+" "+hname);
            pst.setString(15, RoomNo+" "+roomlist);
            pst.execute();
            pst.close();
